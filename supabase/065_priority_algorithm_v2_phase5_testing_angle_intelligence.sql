-- Priority Algorithm v2 Phase 5: Testing Angle intelligence in Normal Mode.
--
-- Preserves Concept ranking, the Phase 1-4 response and history semantics, and
-- the Migration 056 Cram branch. After the Concept winner is fixed, angle need
-- is recomputed from coverage, elapsed time, latest response, and a bounded
-- six-attempt/45-day contradiction signal. Tested angles are capped below the
-- untested score so no prior failure becomes a permanent eligibility gate.

begin;

create index if not exists review_attempts_user_concept_angle_recent_idx
  on public.review_attempts (
    user_id,
    concept_id,
    (lower(btrim(testing_angle))),
    created_at desc,
    id desc
  )
  where testing_angle is not null;
