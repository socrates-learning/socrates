-- Study startup and official Normal selector scale hardening.
--
-- This migration preserves every released Algorithm v2 scoring coefficient,
-- ordering rule, Cram traversal rule, and source-arbitration policy. It changes
-- only when full Question payloads are materialized and how the initial Study
-- Session plus first candidate are returned to the client.

begin;

-- The repeated-lapse input reads the six newest post-reset attempts per
-- learner/Concept. Existing angle history has a different expression key and
-- cannot efficiently provide this ordering once learner history grows.
create index if not exists review_attempts_user_concept_recent_idx
  on public.review_attempts(user_id, concept_id, created_at desc, id desc);

create or replace function public.select_next_study_question_hardened(
...
revoke all on function public.select_next_study_question_hardened(uuid, boolean)
  from public, anon, authenticated;

-- Preserve the released public selector payload exactly. The internal helper
-- carries three additional fields needed to adapt the unified candidate
-- without a second broad resolver pass; clients of the official selector do
-- not observe those internal transport fields.
...
declare
  selection_result jsonb;
begin
  selection_result := public.select_next_study_question_hardened(
    p_study_session_id,
    p_include_debug
  );
...
comment on function public.select_next_study_question_hardened(uuid, boolean) is
  'Internal Concept-first selector that carries the winning Question payload needed by unified selection without a broad resolver rematerialization.';
comment on function public.select_next_study_question(uuid, boolean) is
  'Selects the same Algorithm v2 official Question with the released response contract; Normal ranking is hardened to materialize Questions only within the winning Concept.';
comment on function public.select_next_study_candidate(uuid, boolean) is
  'Returns one discriminated official or personal Study candidate without redundant source-availability resolver passes; source policy and Cram traversal semantics are unchanged.';
...
create or replace function public.start_study_session_with_candidate(
...
comment on function public.start_study_session_with_candidate(uuid, integer, uuid, boolean) is
  'Atomically creates an idempotent client-identified Study Session, captures the existing snapshot, and returns its first unified candidate. Candidate failure rolls back the new Session.';

revoke all on function public.start_study_session_with_candidate(uuid, integer, uuid, boolean)
  from public, anon, authenticated;
grant execute on function public.start_study_session_with_candidate(uuid, integer, uuid, boolean)
  to authenticated;
