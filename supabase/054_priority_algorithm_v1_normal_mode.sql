-- Deterministic Priority Algorithm v1 for normal Study Mode.
--
-- Selection is read-only and proceeds in three stages: Concept, Testing Angle,
-- then Question. Cram Mode deliberately remains outside this selector.
...
begin
  ...
  if target_session.cram_mode then
    raise exception 'Cram selector is not implemented.';
  end if;
  ...
  ranked_concepts as (
    select
      score.*,
      0.20::double precision * score.urgent_review as urgent_review_component,
      score.new_component
        + score.review_component
        + score.angle_component
        + 0.20::double precision * score.urgent_review
  ...
  selected_question_pool as (
    select
      ...
      case
        ...
        when selected.mastery_estimate < 0.40::double precision then 0
        when selected.mastery_estimate <= 0.75::double precision then 1
        else 2
