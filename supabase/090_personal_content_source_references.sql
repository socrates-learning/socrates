-- Optional learner-authored source/reference metadata for personal Concepts
-- and Cards. This remains separate from the official Sources catalog and has
-- no effect on Study eligibility, selection, or learner-state updates.
...
begin
  if to_regclass('public.personal_concepts') is null
    ...
    or to_regprocedure(
      'public.create_personal_concept_overlay(uuid,text,text,uuid,uuid)'
    ) is null
  then
    raise exception 'Personal content and overlay prerequisites are not installed.';
  end if;
end;
$$;

alter table public.personal_concepts
  add column source_reference text;

alter table public.personal_cards
  add column source_reference text;

comment on column public.personal_concepts.source_reference is
  'Optional learner-authored free-text source/reference metadata.';
comment on column public.personal_cards.source_reference is
  'Optional learner-authored free-text source/reference metadata.';

-- Preserve atomic overlay creation while allowing the same optional Concept
-- metadata used by direct personal Concept authoring. The final defaulted
-- argument keeps existing five-argument and named-argument callers valid.
drop function public.create_personal_concept_overlay(
  uuid, text, text, uuid, uuid
);

create function public.create_personal_concept_overlay(
